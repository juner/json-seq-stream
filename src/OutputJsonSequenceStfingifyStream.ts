...
export type OutputJsonSequenceStringifyStreamOptions<T> = {
...
function makeInternalOutputJsonSequenceStringifyStream<T>({ stringify, errorFallback }: OutputJsonSequenceStringifyStreamOptions<T> = {}): {
 ...
 */
export class OutputJsonSequenceStringifyStream<T> extends TransformStream<T, string> {
  constructor(options: OutputJsonSequenceStringifyStreamOptions<T> = {}) {
    const { args } = makeInternalOutputJsonSequenceStringifyStream(options);
    super(...args);
  }
}
